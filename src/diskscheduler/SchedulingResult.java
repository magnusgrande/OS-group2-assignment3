 ...
 *
 * <ul>
 *   <li>the service order (cylinder numbers in the order they were serviced),
 *   <li>the full head path (every position the head moved to, including the initial head position
 *       and any intermediate waypoints such as the disk end in SCAN),
 *   <li>per-step movement descriptions for step-by-step output, and
 *   <li>the total number of cylinders the head travelled.
 * </ul>
 */
public class SchedulingResult {
  ...
  /**
   * Returns every position the disk head occupied during the simulation, starting with the initial
   * head position. For SCAN this includes the disk-end waypoint (cylinder 0 or 199).
   *
  ...
  /**
   * Returns human-readable descriptions of every individual head movement, one entry per step (e.g.
   * {@code "53 -> 65 (dist: 12)"}).
   *
  ...
  /**
   * Computes the average seek distance as total head movement divided by the number of serviced
   * requests.
   *
   * @return average seek distance in cylinders, or {@code 0.0} if no requests were serviced
   */
