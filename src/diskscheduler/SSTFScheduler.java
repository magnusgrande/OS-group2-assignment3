 ...
 *
 * <p>At each step, the pending request nearest to the current head position is
 * selected next. SSTF minimises individual seek distances and therefore
 * typically achieves a lower total head movement than FCFS. However, it can
 * cause <em>starvation</em>: if nearby requests keep arriving, distant requests
 * may be delayed indefinitely.
 *
 * <p><b>Tie-breaking rule:</b> when two pending requests are equally close to
 * the current head, the one that appears earlier in the original request queue
 * is selected first.
 */
public class SSTFScheduler {
     ...
     *
     * <p>The disk head starts at {@code initialHead} and greedily moves to the
     * closest pending cylinder at each step until all requests have been serviced.
     *
