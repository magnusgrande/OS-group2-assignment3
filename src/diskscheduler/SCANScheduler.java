 ...
 *
 * <p>The disk head sweeps in the given initial direction, servicing all requests it encounters
 * along the way. When the head reaches the end of the disk (cylinder {@value MIN_CYLINDER} when
 * travelling left, or {@value MAX_CYLINDER} when travelling right), it reverses direction and
 * sweeps back, servicing any remaining requests. This provides more uniform wait times than SSTF
 * and avoids indefinite starvation.
 *
 * <p><b>Disk end behaviour:</b> the head always travels to the physical end of the disk before
 * reversing, even when no requests lie beyond the last one in the current direction — provided
 * there are still requests to service in the opposite direction.
 *
 * <p><b>Assumptions:</b>
 *
 * <ul>
 *   <li>Disk cylinders are numbered {@value MIN_CYLINDER} through {@value MAX_CYLINDER} inclusive.
 *   <li>A request at exactly the current head position is treated as being in the current sweep
 *       direction (distance = 0, serviced first).
 * </ul>
 */
public class ScanScheduler {
   ...
   * @param initialDirection initial sweep direction: {@code "left"} or {@code "right"}
   *     (case-insensitive)
   * @param requests cylinder numbers to service
   * @return a {@link SchedulingResult} containing the service order, full head path (including
   *     disk-end waypoint), per-step movements, and total head movement
   * @throws IllegalArgumentException if {@code initialDirection} is not {@code "left"} or {@code
   *     "right"}
   */
  public SchedulingResult simulate(int initialHead, String initialDirection, int[] requests) {
    if (!initialDirection.equalsIgnoreCase("left") && !initialDirection.equalsIgnoreCase("right")) {
    ...
    if (!secondPass.isEmpty()) {
      ...
      if (current != endOfDisk) {
        ...
        totalMovement += distance;
        steps.add(current + " -> " + endOfDisk + " [end of disk, dist: " + distance + "]");
